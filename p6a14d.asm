...
;	variables filtro------------------------------------
n1			equ		0x10		; 1林ectura
n2			equ		0x11		; 2林ectura
n3			equ		0x12		; 3林ectura
n4			equ		0x13		; 4林ectura
n5			equ		0x14		; 5林ectura
n6			equ		0x15		; 6林ectura
n7			equ		0x16		; 7林ectura
n8			equ		0x17		; 8林ectura
...
Tmr0Int		movwf 	WTemp		;Copia W a W_TEMP
			...
			movwf 	StaTemp 	;Guarda STATUS en STATUS_TEMP a trav廥 de W	
;---Instrucciones tratamiento interrupci鏮 por desbordamiento del timer
			bcf     INTCON,GIE  ;Deshabilitar interrupciones
			call	Int1cs		;Prepara el temporizados para nueva interrupci鏮 en 10ms.
...
NDelay1		swapf	StaTemp,0	;Copia STATUS_TEMP a W con intercambio de nibbles
			movwf	StaTemp 	;Restaura STATUS_TEMP a STATUS a trav廥 de W
			...
			bcf		INTCON,2	;Limpieza del bit de interrupci鏮
			retfie				;recupera de la pila la direcci鏮 en la que hab燰 sido interrumpido
...
ringbuf		movf	ind,0 		;carga el valor del indice en W
			...
			movwf	INDF		;carga de w en el INDF(indf apunta a la direcci鏮 de FSR)
...
EepromW		movlw	0x00
			movwf	EEADR		; escribe la direcci鏮 en eeadr
			movf	average,0
...
Multip		clrf	DH
			...
			movf	MultipB,0
			movwf	DL
			Movlw	0x09	;inicializaci鏮 variable indexado i, se inicia a 9 para que ejecute el ciclo 8 veces
			Movwf	i;
